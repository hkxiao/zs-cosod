...
class Solver(object):
    ...
    def test(self):
        ...
        for i, data_batch in enumerate(self.test_loader):
            images, name, group = data_batch['image'], data_batch['name'][0], data_batch['group'][0]
            with torch.no_grad():
                ...
                import os
                # print(group)
                if not os.path.exists(os.path.join(self.config.test_fold,group)):
                    os.mkdir(os.path.join(self.config.test_fold,group))
                # print(self.config.test_fold)
                # print(name)
                # print(os.path.join(self.config.test_fold,group ,name))
                #raise NameError()
                cv2.imwrite(os.path.join(self.config.test_fold,group ,name), multi_fuse)
        time_e = time.time()
