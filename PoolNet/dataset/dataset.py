...
class ImageDataTest(data.Dataset):
    def __init__(self, data_root, data_list):
        ...
        groups = sorted(os.listdir(self.data_root+'/img'))
        
        for group in groups:
            imgs = sorted(os.listdir(self.data_root+'/img/'+group))
            for img in imgs: 
                self.image_list.append(self.data_root+'/img/'+group+'/'+img)
    ...
    def __getitem__(self, item):
        ...
        group = self.image_list[item].split('/')[-2]
    
        return {'image': image,'group':group ,'name': self.image_list[item].split('/')[-1], 'size': im_size}
